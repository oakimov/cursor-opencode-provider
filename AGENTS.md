...
| Host paths | `src/context/paths.ts` | Host cache root (`cacheDir` / OCP detect / host heuristic); project metadata under `<host-cache>/projects/<slug>/` |
...
| CreatePlan bridge | `src/protocol/create-plan.ts`, `hostPlansDir` in `src/context/paths.ts` | Cursor CreatePlan (#7) → plain markdown under host-calculated `plans/` (path-bridge project-config dir, else host global data) |
...
- **SwitchMode bridge:** Cursor's native SwitchMode is an inbound `InteractionQuery` (#4) with `SwitchModeArgs{target_mode_id, explanation?, tool_call_id}`; the CLI blocks until `approved{}` or `rejected{reason}` (`"Mode switch rejected by user"` on decline) — there is no async variant. OpenCode's only model-callable primary switches are `plan_enter` (→ plan) and `plan_exit` (→ build), so the provider translates rather than refusing: `plan`/`spec` → `plan_enter`; **every other non-empty target** (including `agent`, `build`, `chat`, `debug`, `edit`, `background`, `multitask`, `triage`, `project`, and unknowns) → `plan_exit`. After approval, the provider records the Cursor unified mode per OpenCode session and injects a CLI-shaped `<system_reminder>` (Ask/Debug/Agent/Cloud/Multitask/Triage/Project/Plan, adapted to OpenCode `task`/`question`) on subsequent Runs via `takeActiveCursorModeReminder`. Do not spawn `task`/subagents for SwitchMode itself — mode reminders may instruct the model to use `task` afterward. **A missing plan tool is not a refusal.** Upstream OpenCode has `PlanEnterTool` commented out (`fa559b038`) and registers `plan_exit` only under `OPENCODE_EXPERIMENTAL_PLAN_MODE` + CLI client, and OCP only *relabels* advertised tools (it never invents them), so MiMo/Kilo usually advertise neither. `resolveSwitchModeBridge` therefore resolves three tiers, keyed solely on the advertised catalog under canonical names — never on host identity or model id: (1) **native** — the matching plan tool is advertised → bridge to it, holding the query open until it returns (AskQuestion-shaped deferred reply); (2) **approve** — entering plan/spec with no `plan_enter` → reply `approved{}` immediately, needing zero host tools, because the injected `<system_reminder>` *is* the contract (Cursor CLI's own plan mode is likewise prompt-enforced); (3) **question** — leaving plan mode with no `plan_exit` → bridge to the host `question` tool carrying upstream `PlanExitTool`'s own prompt, so the user still approves before any execution starts; only an explicit "Yes" approves, and an unanswered/dismissed prompt keeps the model planning. Reject only when leaving plan mode and neither `plan_exit` nor `question` exists (plain pi) — the reason must name both. Rationale: OpenCode's plan tools are not mode flags; each asks via the Question service and, on Yes, injects a synthetic user message carrying `agent:"plan"|"build"` that `createUserMessage` turns into a `setAgentModel` primary-agent switch. A provider cannot reach `setAgentModel`, but the approval gate and the behavioural contract are both reachable, so emulation is faithful to everything a provider can influence. Never withhold `edit`/`write` to harden emulated plan mode — that breaks the frozen-RequestContext invariant and blocks writing the plan itself. The plan reminder must name `plan_exit` only when advertised, else the reachable Cursor-native SwitchMode back to `agent`. Display `switch_mode_tool_call` (#25) is a separate transcript record (`SwitchModeResult` success/error/rejected; CLI hardcodes `from_mode_id=""`); the display mapper uses the same mode mapping and stays non-replayed.
- **CreatePlan bridge:** Cursor's `create_plan_request_query` (#7) writes a real plan file — not Cursor's `~/.cursor/plans` and not Cursor YAML frontmatter. Ordinary hosts return a `file://` `plan_uri`; omp native plan mode returns its session-local `local://<slug>-plan.md` URI through one `cursor_plan_stage` host tool, but only after this provider has approved the bridge's `plan_enter` continuation. That tool stages the file, opens pi-bridge's own extension-UI approval/refinement selector, restores tools and queues execution on approval, and returns the result that completes Cursor's held query. Do not split it into `cursor_plan_stage` + `write xd://propose`: npm omp runs its bundled `InteractiveMode`, so source-only UI changes in another checkout never receive that proposal, and the successful-but-unowned proposal ends in an empty-stop retry loop. Outside the bridge-entered state, fall back to the ordinary host plan write rather than dispatch an inactive native proposal. Ordinary location follows OpenCode's `Session.plan` shape via `hostPlansDir`: inside a git worktree, `<worktree>/<primary project-config dir>/plans/` where the project-config dir comes from the path bridge (`opencodeProjectConfigDirs` → `.opencode` / `.mimocode` / `.kilo` / …); with no VCS, `<hostGlobalDataDir>/plans/`. Never hardcode `.opencode/plans/` — that is only OpenCode's default when the bridge is absent. Body is plain markdown (`# title`, overview, plan, optional `## Todos`). Empty / missing args keep the CLI empty-`plan_uri` success ack. Display `create_plan_tool_call` still mirrors into `todowrite` separately and does not write the file.
...
| `XDG_CACHE_HOME` | Base for the explicitly detected or install-path host; otherwise OpenCode `…/opencode`; OMP uses `…/omp` after its XDG migration |
| `PI_CODING_AGENT_DIR` | Custom Pi/OMP agent root; private provider cache defaults below `<agent-root>/cache/cursor-opencode/` |
| `PI_CONFIG_DIR` | Custom Pi/OMP config root name used when deriving the default agent root |
| `<host-cache>/cursor-models.json` | Model cache under resolved host cache (OpenCode default `~/.cache/opencode/`; Pi/OMP use their own agent cache) |
