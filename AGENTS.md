...
- **`TurnEnded` token accounting:** Cursor's current CLI treats each event as request-local. Live continuation counters can decrease between turns, so never subtract a prior turn or persist an accounting baseline. `input_tokens` is total input **including** cache reads/writes; derive non-cached input as `max(input_tokens - cache_read - cache_write, 0)`. `output_tokens` includes reasoning; derive visible output as `max(output_tokens - reasoning_tokens, 0)`. AI SDK V3 receives `{ total, noCache, cacheRead, cacheWrite }` / `{ total, text, reasoning }`. Emit zero at held-Run tool boundaries and the complete final event exactly once; preserve raw fields in `providerMetadata.cursor`.
...
| `<host-cache>/cursor-conversations/*.pb.gz` | One atomic gzip-compressed protobuf restart snapshot per stable OpenCode session (checkpoint, reachable blobs, stable context base, lifecycle tool fallback, compaction marker), updated at successful `TurnEnded`; records expire after 24h |
