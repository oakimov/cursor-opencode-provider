...
- **Prompt-cache context:** freeze and persist the stable `RequestContext` base (rules, env, repository, git, layout) per Cursor `conversation_id`; rediscover tools/MCP, skills, subagents, and plugin metadata every Run and overlay them deterministically. Reuse the prior materialized object when its protobuf bytes are unchanged. A restored last-real tool catalog is for lifecycle turns such as compaction only—never ordinary restricted/no-tool turns. Deduplicate concurrent first builds and clear the base with checkpoints/blobs on compaction reset or binding eviction.
