...
- **Host web tools use collision-safe aliases** — when the current tool catalog has an exact web search/fetch tool, or one unique flattened MCP match, Cursor sees `custom_websearch` / `custom_webfetch`. The held Run maps the selected alias back to the exact original tool name before host execution, preserving its schema, permission check, and correlated result. Ambiguous search/fetch providers are left unchanged rather than guessed.
