...
| OpenCode token totals grow once per tool call | Upgrade to a version containing the held-Run usage fix. Cursor's final `TurnEnded` counters are request-local, while OpenCode sums every AI SDK step. Fixed versions emit zero usage at intermediate tool boundaries and emit the final request counters exactly once when the held Run closes. The raw counters remain in `providerMetadata.cursor`. |
...
- **`TurnEnded` usage is request-local** — live continuation captures show counters can decrease between turns (`output_tokens` 633 → 294 and `cache_read` 36,608 → 27,392), and Cursor CLI consumes each event directly. `input_tokens` already includes `cache_read`/`cache_write`, while `output_tokens` already includes `reasoning_tokens`; the provider derives the non-cached and visible-text subsets instead of adding those fields twice. OpenCode sums every AI SDK step, so intermediate tool-call boundaries report zero usage and the successful final `TurnEnded` reports that request's complete counters exactly once. Raw values remain in `providerMetadata.cursor` for diagnostics. Cursor consistently sends `cache_write=0` in captured Runs, so no alternate write count is available.
