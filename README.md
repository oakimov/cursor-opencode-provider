...
4. Host-specific fallback: Pi `~/.pi/agent/cache/cursor-opencode/`, OMP `~/.omp/agent/cache/cursor-opencode/`, or OpenCode `~/.cache/opencode/`
...
|------|--------------------|-------|
| Model, version, and 24-hour conversation **cache** | `~/.cache/opencode/` | Pi: `~/.pi/agent/cache/cursor-opencode/`; OMP: `~/.omp/agent/cache/cursor-opencode/`; MiMo: `$MIMOCODE_HOME/cache` or `~/.cache/mimocode/`; Kilo: `~/.cache/kilo/` |
...
- **Cursor-native CreatePlan writes a host plan file** — when Cursor raises `create_plan_request_query`, the provider writes plain markdown (no Cursor YAML frontmatter, no `~/.cursor/plans`). On omp through pi-bridge, one `cursor_plan_stage` call stages `local://<slug>-plan.md` and opens pi-bridge's interactive approval/refinement selector while Cursor's query stays open. Approval restores the pre-plan tools, clears the provider's Cursor plan reminder, and queues execution; refinement stays in plan mode. Other hosts use their calculated plans directory: the primary project-config dir from the path bridge (`opencodeProjectConfigDirs` → `.opencode` / `.mimocode` / `.kilo` / …) plus `plans/` inside a git worktree, or `<hostGlobalDataDir>/plans/` when there is no VCS. The reply carries the resulting plan URI. Empty args still get the CLI empty-`plan_uri` success ack. Display `create_plan_tool_call` continues to mirror into `todowrite` separately.
