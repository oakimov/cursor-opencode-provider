...
- **MCP tools keep their upstream server id** — OpenCode builtins are advertised under a synthetic `opencode` MCP server; real MCP tools (`github_create_pull_request`, …) are grouped into separate `mcp_descriptors` / `provider_identifier`s (`github`, …) so models can discover “github mcp” instead of a single flattened `opencode` catalog. Exec still routes back through this provider and reconstructs the full OpenCode tool id.
...
- **Cursor-native interactions are headless** — Cursor UI/approval requests cannot be surfaced through the AI SDK provider interface. Web access, questions, mode changes, PR/MCP/image/SCM actions are declined so they stay behind OpenCode's own tools and permissions; plan/environment lifecycle requests receive the same non-interactive acknowledgements used by Cursor's headless client. Unknown future interaction variants fail the turn explicitly instead of hanging the Run stream.
- **Compaction resets Cursor conversation state** — OpenCode compaction/summary turns advertise no tools. On those turns the provider mints a fresh Cursor `conversation_id`, drops the prior checkpoint + KV blobs, and seeds `conversation_state` from OpenCode's compacted prompt history so `TurnEnded.cache_read` cannot keep the session overflowing after a successful local compact.
