...
- **Cursor-native CreatePlan writes a host plan file** — when Cursor raises `create_plan_request_query`, the provider writes plain markdown (no Cursor YAML frontmatter, no `~/.cursor/plans`) under the host's calculated plans directory: the primary project-config dir from the path bridge (`opencodeProjectConfigDirs` → `.opencode` / `.mimocode` / `.kilo` / …) plus `plans/` inside a git worktree, or `<hostGlobalDataDir>/plans/` when there is no VCS. The reply carries a real `file://` `plan_uri`. Empty args still get the CLI empty-`plan_uri` success ack. Display `create_plan_tool_call` continues to mirror into `todowrite` separately.
